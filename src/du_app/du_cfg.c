...
void getPrimaryIp(char* host)
{
   ...
   printf("intf %s", INTERFACE_TO_RIC);
   for(ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, ++n)
   {
      // skip tun interface created by srslte
      if (strcmp(ifa->ifa_name, SRSEPC_TUN_INTERFACE) == 0 || strcmp(ifa->ifa_name, INTERFACE_TO_USRP) == 0) {
        continue;
      }
      ...
      family = ifa->ifa_addr->sa_family;
        ...
        if (family == AF_INET && strcmp(ifa->ifa_name, INTERFACE_TO_RIC) == 0) {
